...
jest.mock('../../../lib/index', () => {
  ...
  return {
    pict: jest.fn((model, options) => Promise.resolve(generateTestCases(model, options))),
    strings: jest.fn((model, options) => Promise.resolve(generateTestCases(model, options))),
    pict: {
      stats: jest.fn((model, options) => Promise.resolve(generateStatistics(model, options)))
    },
    strings: {
      stats: jest.fn((model, options) => Promise.resolve(generateStatistics(model, options)))
    }
...
describe('Statistics E2E Workflow Tests', () => {
  ...
  const constraints = [
    '[location] = "USA" => [paymentSystem] <> "WebMoney"'
  ];
  ...
  test('Complete statistics workflow with different order values', async () => {
    ...
    expect(generateResponse.body.statistics).toHaveProperty('coveragePercentage', 100);
    ...
    expect(statsResponse2Way.body.statistics).toHaveProperty('constraintReduction');
    ...
    const threeWayTheoretical = statsResponse3Way.body.statistics.theoreticalMax;
