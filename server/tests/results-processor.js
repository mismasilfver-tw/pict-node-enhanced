...
module.exports = function (results) {
  ...
  const totalPassed =
    unitTests.passed + integrationTests.passed + e2eTests.passed;
  const totalFailed =
    unitTests.failed + integrationTests.failed + e2eTests.failed;

  // Calculate percentages for the pyramid
  const unitPercent =
    totalTests > 0 ? ((unitTests.total / totalTests) * 100).toFixed(1) : 0;
  const integrationPercent =
    totalTests > 0
      ? ((integrationTests.total / totalTests) * 100).toFixed(1)
      : 0;
  const e2ePercent =
    totalTests > 0 ? ((e2eTests.total / totalTests) * 100).toFixed(1) : 0;

  // Create a visual representation of the test pyramid
  console.log("\n");
  console.log("=== TEST PYRAMID DISTRIBUTION ===");
  console.log(
    `E2E Tests:          ${"█".repeat(Math.ceil(e2ePercent / 5))} ${e2ePercent}% (${e2eTests.total} tests)`,
  );
  console.log(
    `Integration Tests:  ${"█".repeat(Math.ceil(integrationPercent / 5))} ${integrationPercent}% (${integrationTests.total} tests)`,
  );
  console.log(
    `Unit Tests:         ${"█".repeat(Math.ceil(unitPercent / 5))} ${unitPercent}% (${unitTests.total} tests)`,
  );
  console.log("\n");
  ...
  if (totalTests >= 10) {
    ...
    if (unitPercent < 60) {
      console.log(
        "⚠️  Unit tests are underrepresented. Consider adding more unit tests.",
      );
    }

    if (e2ePercent > 15) {
      console.log(
        "⚠️  E2E tests are overrepresented. Consider focusing more on unit tests.",
      );
    }

    if (unitPercent >= 60 && integrationPercent >= 15 && e2ePercent <= 15) {
      console.log(
        "✅ Test distribution follows the test pyramid principle well!",
      );
    }
