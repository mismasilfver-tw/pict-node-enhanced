...
/**
 * Component for testing validation scenarios in the constraint builder UI
 */
const ValidationTests = () => {
  ...
  const [invalidComboParam, setInvalidComboParam] = useState(null as string | null);
  const [invalidComboOperator, setInvalidComboOperator] = useState(null as OperatorType | null);
  ...
  useEffect(() => {
    if (typeMismatchCondition.parameterKey && 
        typeMismatchCondition.operator && 
        typeMismatchCondition.values.length > 0) {
      
      const param = sampleParameters.find(p => p.key === typeMismatchCondition.parameterKey);
      ...
      if (numericOperators.includes(typeMismatchCondition.operator)) {
        const allNumeric = typeMismatchCondition.values.every(v => typeof v === 'number' || !isNaN(Number(v)));
  ...
  useEffect(() => {
    if (syntaxCondition.parameterKey && 
        syntaxCondition.operator && 
        syntaxCondition.values.length > 0) {
      
      const param = sampleParameters.find(p => p.key === syntaxCondition.parameterKey);
      const isNumeric = param?.values.every(v => typeof v === 'number');
      ...
      try {
        ...
        if (syntaxCondition.operator === 'IN') {
          ...
          valueStr = isNumeric ? syntaxCondition.values[0] : `"${syntaxCondition.values[0]}"`;
        }
        ...
        if (syntaxCondition.parameterKey.includes(' ') || 
    ...
    <Container>
        ...
        <Card.Body>
          ...
          {invalidComboParam && invalidComboOperator && (
            <Alert variant={invalidComboValidation.type}>
              <strong>Validation Result:</strong> {invalidComboValidation.message}
            </Alert>
          )}
          ...
          <div className="mt-3">
            ...
            <ul>
              ...
              <li>Select 'fileSystem' and {'>'} operator - should show error</li>
      ...
      <Card className="mb-4">
        ...
        <Card.Body>
          ...
          {typeMismatchCondition.parameterKey && typeMismatchCondition.operator && typeMismatchCondition.values.length > 0 && (
            <Alert variant={typeMismatchValidation.type} className="mt-3">
              <strong>Type Validation:</strong> {typeMismatchValidation.message}
            </Alert>
          )}
          ...
          <div className="mt-3">
            <h6>Test Cases:</h6>
            <ul>
              <li>Select 'mixedTypes' parameter - should warn about inconsistent types</li>
              <li>Select 'size' parameter with {'>'} operator and select a non-numeric value - should show error</li>
      ...
      <Card className="mb-4">
        <Card.Header>Syntax Validation Tests</Card.Header>
        <Card.Body>
          <ConditionBuilder
            parameters={sampleParameters}
